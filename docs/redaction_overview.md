...
- В тексте запросов сохраняется структура запроса, но URI credentials, логины, пароли, токены и известные форматы API-ключей заменяются маркерами.
- Фрагменты Model Lab проходят тот же sanitizer до формирования expert prompt.
...
- Маскирование применяется ко всему содержимому файла, которое отправляется в LLM, включая повторные экспертные вызовы Model Lab.
...
## Ограничения
- Полное маскирование применяется к строковым литералам и комментариям; для сохраняемого текста запросов дополнительно применяются высокоточные шаблоны credentials и токенов.
