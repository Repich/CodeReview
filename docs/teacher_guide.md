...
1) Откройте запуск и включите контекст (он включён по умолчанию).  
2) Выделите фрагмент кода в блоке «Контекст».  
3) Появится карточка «Выделение для новой нормы» с файлом и строками.  
4) Нажмите `Создать норму`.  
5) Форма откроется с предзаполненным полем «Пример/выдержка» (текст выделения).  
6) Заполните обязательные поля и сохраните.

Важно: выделение берётся из блока «Контекст», где строки имеют формат `NNN: код`.
Если выделять текст вне контекста, карточка не появится.
